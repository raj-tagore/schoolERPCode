...
class Parent(models.Model):
    account = models.OneToOneField(Account, on_delete=models.CASCADE, related_name='parent_account')
...
class Teacher(models.Model):
    account = models.OneToOneField(Account, on_delete=models.CASCADE, related_name='teacher_account')
    identifier = models.BigIntegerField('ID')
    qualifications = models.TextField('Qualifications', blank=True)
    phone = models.CharField('Phone number', max_length=20, blank=True)
    whatsapp = models.CharField('WhatsApp number', max_length=20, blank=True)
    docs = models.ManyToManyField(Attachment, related_name='teacher_docs')

class Student(models.Model):
    account = models.OneToOneField(Account, on_delete=models.CASCADE, related_name='student_account')
    # Basic Fields
